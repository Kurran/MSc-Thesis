...
% Debug
function plot_function(mu_tilde, T_tilde, x_max, N)
  x = linspace(0, x_max, N+1);
  % fermi-dirac function
  fd = 1 .+ exp((x.-mu_tilde)./T_tilde);
  fd = 1 ./ fd;
...
entropy = zeros(1, length(T_tilde) - 1);

figure;
hold on;

filename = "entropy_vs_temperature.dat";
fid = fopen (filename, "w");
for i=2:length(mu_tilde)
  plot_function(mu_tilde(i), T_tilde(i), x_max, N);
  entropy(i-1) = calculate_integral(mu_tilde(i), T_tilde(i), x_max, N);
  fprintf(fid, "%f    %f\n",T_tilde(i), entropy(i-1));
endfor;
fclose (fid);
hold off;
